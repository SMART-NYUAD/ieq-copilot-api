...
**Known limitation — threshold *attribution* is still unreliable.** With several sources per metric the model reliably gets the direction right (elevated vs fine) but frequently names the wrong source for the number it applied: PM2.5 at 17.2 µg/m³ reported as "above the EPA daily standard" (EPA's is 35; it is WHO's 15 that is exceeded), VOC at 0.08 ppm as exceeding the UBA precautionary level (0.194; it is WHO's 0.061 that is exceeded). Tightening the prompt further made it *worse*, not better — an added "cite the source that publishes the number you applied" clause produced a fabricated "RESET Air Grade A threshold of 0.07 ppm" and was reverted. The durable fix is not prose: compute the exceedances in code from `threshold_value`/`threshold_unit` against the reading, and hand the model a pre-resolved per-metric verdict with its source index, the same way `metric_planning` moved metric selection out of prose and into a table.

Two guideline records carry a NULL `threshold_value` **on purpose** and must not be "filled in": ASHRAE 62.1 defines ventilation rates, not a CO2 ppm limit (asserting one is the exact false claim `GUIDELINE_CITATIONS` rule 4 exists to prevent), and ASHRAE 55 defines thermal comfort through PMV rather than a fixed temperature. VOC is also a live unit mismatch — readings are in ppm, every available threshold is in µg/m³, and converting between them needs a molar-mass assumption, so the directive tells the model to say they cannot be compared rather than convert.

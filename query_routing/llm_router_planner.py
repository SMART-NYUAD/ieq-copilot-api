...
_SYSTEM_PROMPT = (
    ...
    "'it', 'that', 'this', 'those') replaced with what it refers to from Prior "
    ...
    "EXCEPTION — contrastive place references ('over there', 'that room', 'the other lab', "
    ...
    "— keeps the INTENT of the turn that produced that answer: after an `ifc_model_qa` answer about "
    "a door width, 'give me that in centimeters' is still `ifc_model_qa` (resolved: 'what is the "
    "width of the main door in centimetres?'), NOT a definition question. "
    "A bare calendar day with NO month — 'what about on the 11th?', "
    ...
    "Clarify when: the question uses a CONTRASTIVE place reference — 'over there', 'that room', "
    "'the other lab', 'elsewhere', 'somewhere else' — which points at a DIFFERENT space than the "
    "one being discussed, so a lab in Prior conversation does NOT resolve it and reusing that lab "
    "silently answers about the wrong space ('what about over there?' → ask which space, even "
    "when smart_lab is in Prior conversation); the question names two incompatible scopes; or it "
    "is so vague that no metric or topic can be identified ('how is it?', 'is it fine?') with "
    "nothing in Prior conversation to resolve it. "
